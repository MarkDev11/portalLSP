...
@section('content')
    ...
    <section style="background: linear-gradient(to right, #0f172a 0%, #0f172a 70%, var(--color-accent) 100%);">
        <div class="max-w-6xl mx-auto px-6 lg:px-8 py-20 md:py-28">
            <div class="reveal">
                ...
                <h1 class="text-3xl md:text-5xl font-bold text-white heading-tight display mb-4">{{ $tentangKami->hero_title }}</h1>
                <div class="w-12 h-0.5 mb-6" style="background-color: var(--color-accent);"></div>
                <p class="text-ink-300 body-large max-w-2xl">{{ $tentangKami->hero_subtitle }}</p>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-6xl mx-auto px-6 lg:px-8">
            <div class="grid lg:grid-cols-2 gap-16 items-center">
                <div class="reveal">
                    ...
                    <h2 class="text-3xl md:text-[2.5rem] font-bold text-ink-900 heading-tight display mb-6">{{ $tentangKami->about_title }}</h2>
                    <div class="line-accent mb-8"></div>
                    <div class="space-y-4 text-ink-500 leading-relaxed">
                        {!! $tentangKami->about_content !!}
                    </div>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="relative border border-ink-200 bg-white aspect-[4/3] overflow-hidden z-10">
                            @if($landingPage->tentang_image)
                                <img src="{{ asset('storage/' . $landingPage->tentang_image) }}" alt="{{ $landingPage->tentang_title }}" class="w-full h-full object-cover">
                            @endif
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-5">
                                <div class="bg-white/20 backdrop-blur-md border border-white/30 px-4 py-3 flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-semibold text-white">{{ $tentangKami->license_title }}</div>
    ...
    <section class="py-24 md:py-32 bg-white">
        <div class="max-w-6xl mx-auto px-6 lg:px-8">
            ...
            <div class="lg:col-span-4 reveal">
                ...
                <h2 class="text-3xl md:text-[2.5rem] font-bold text-ink-900 heading-tight display mb-6">{{ $tentangKami->tujuan_title }}</h2>
                <div class="line-accent mb-8"></div>
                <p class="text-ink-500 leading-relaxed">
                    {{ $tentangKami->tujuan_subtitle }}
                </p>
            </div>
                
                <!-- Right: Tujuan list -->
                <div class="lg:col-span-8 space-y-0">
                    @foreach($tentangKami->tujuan_items as $index => $tujuan)
    ...
    <section class="py-20 md:py-28" style="background: linear-gradient(to right, var(--color-accent) 0%, #0f172a 50%, var(--color-accent) 100%);">
        <div class="max-w-4xl mx-auto px-6 lg:px-8 text-center reveal">
            <h2 class="text-3xl md:text-4xl font-bold text-white heading-tight display mb-6">{{ $tentangKami->cta_title }}</h2>
            <p class="text-ink-400 body-large mb-10 max-w-2xl mx-auto">{{ $tentangKami->cta_description }}</p>
